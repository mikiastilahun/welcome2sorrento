<script lang="ts">
	...
	// SEO from CMS
	const seoTitle = aboutData.seo?.metaTitle || 'About Us - Our Story & Services | Welcome2Sorrento';
	...
	const seoKeywords = aboutData.seo?.keywords || '';

	// Get images from CMS
	const heroImage = aboutData.heroImage?.asset?.url || '';
	const storyImage = aboutData.story?.image?.asset?.url || '';
	const servicesImage = aboutData.servicesSection?.image?.asset?.url || '';
	const ctaBackgroundImage = aboutData.cta?.backgroundImage?.asset?.url || '';
	...
	const servicesList =
		aboutData.services && aboutData.services.length > 0
			? aboutData.services.map((s) => s.title)
			: defaultServices;

	// CTA from CMS
	const ctaHeading = aboutData.cta?.heading || 'Ready to Plan Your Trip?';
	const ctaDescription =
		aboutData.cta?.description || 'Let us help you create unforgettable memories in Sorrento';
	const ctaButtonText = aboutData.cta?.buttonText || 'Get in Touch';
	const ctaButtonLink = aboutData.cta?.buttonLink || '/contact';
</script>
				...
				<div class="grid grid-cols-1 items-center gap-12 lg:grid-cols-2">
					{#if storyImage}
						<div class="overflow-hidden rounded-xl shadow-lg">
							<img src={storyImage} alt="Sorrento" class="h-[400px] w-full object-cover" />
						</div>
					{/if}
					...
					<div class="space-y-6 text-lg leading-relaxed text-[var(--stone)]">
						...
						{#if aboutData.mission}
							<p
								class="mt-6 rounded-lg border-l-4 border-[var(--azure)] bg-white p-4 italic text-[var(--charcoal)]"
							>
								"{aboutData.mission}"
							</p>
						{/if}
	...
	<section class="bg-white py-16" id="services">
		<div class="container mx-auto px-4 sm:px-6 lg:px-8">
			<div class="scroll-reveal mx-auto max-w-6xl" use:reveal>
				<div class="mb-10 text-center">
					<h2 class="heading-serif mb-4 text-3xl font-semibold text-[var(--charcoal)] sm:text-4xl">
						{aboutData.servicesSection?.heading || 'WhatsApp Booking Service'}
					</h2>
	...
	<section class="relative py-20">
		<div class="absolute inset-0">
			{#if ctaBackgroundImage}
				<img src={ctaBackgroundImage} alt="Sorrento coastline" class="h-full w-full object-cover" />
			{/if}
