					...
					<a href={destination.link} class="group block h-full">
						...
						>
							...
							<div class="relative {!destination.description ? 'h-full' : 'h-72'} overflow-hidden">
								...
								<!-- Gradient overlay -->
								<div
									class="from-[var(--charcoal)]/70 via-[var(--charcoal)]/30 absolute inset-0 bg-gradient-to-t to-transparent"
								></div>

								<!-- Italia stamp -->
								<div class="absolute left-4 top-4 z-10 rotate-[-5deg] opacity-80">
									<div
										class="rounded border-2 border-dashed border-white/50 bg-white/10 px-3 py-1 backdrop-blur-sm"
									>
										<span class="font-serif text-xs font-bold uppercase tracking-[0.2em] text-white"
											>ITALIA</span
										>
									</div>
								</div>
